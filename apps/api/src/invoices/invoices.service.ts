...
export class InvoicesService {
    ...
    async createFromOrder(orgId: string, orderId: string) {
        const order = await this.prisma.order.findFirst({
            where: { id: orderId, orgId },
            include: { items: { include: { product: true } }, customer: true }
        });
        if (!order) throw new NotFoundException({ code: 'NOT_FOUND', message: 'Order not found' });
        if (order.invoiceId) throw new NotFoundException({ code: 'CONFLICT', message: 'Order already has an invoice' });

        const org = await this.prisma.organization.findUnique({ where: { id: orgId } });
        const sameState = (org?.address ?? '') === (order.customer.state ?? '') || true;
        ...
        const itemsData = order.items.map(item => {
            ...
            return {
                productId: item.productId, description: product.name,
                hsnCode: product.hsnCode, quantity: item.quantity,
                unit: item.unit, price: item.price, discount: item.discount, taxableAmt: taxable,
                gstRate: product.gstRate, ...gst, cessRate: product.cessRate ?? 0, totalAmount: total,
            };
        });

        const totalAmount = order.totalAmount + cgstAmount + sgstAmount + igstAmount + cessAmount;
        const invoiceNumber = await this.generateInvoiceNumber(orgId);
        ...
        const invoice = await this.prisma.$transaction(async (tx: any) => {
            const inv = await tx.invoice.create({
                data: {
                    orgId, invoiceNumber, customerId: order.customerId,
                    invoiceDate: new Date(), dueDate, notes: order.notes,
                    subtotal: order.totalAmount + order.discountAmount,
                    discountAmount: order.discountAmount,
                    taxableAmount: order.totalAmount,
            ...
            await tx.order.update({ where: { id: order.id }, data: { invoiceId: inv.id } });
            await tx.customer.update({ where: { id: order.customerId }, data: { outstandingAmount: { increment: totalAmount } } });
            return inv;
        });

        await this.queue.addToQueue('invoice', 'generate-pdf', { invoiceId: invoice.id });
        this.logger.log(`Queued PDF generation for invoice ${invoice.id} from order ${order.id}`);
