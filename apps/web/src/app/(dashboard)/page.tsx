...
export default function DashboardPage() {
    ...
    const outstanding = d?.collections?.totalOutstanding ?? 0;
    ...
    const alerts = useMemo(() => {
        ...
        if (lowStock > 0) items.push({ type: "stockout", message: `${lowStock} products below reorder level`, action: "View", color: "var(--red)", href: "/inventory" });
        if (outstanding > 0) items.push({ type: "overdue", message: `${formatINR(outstanding)} total outstanding from customers`, action: "Collect", color: "var(--warning)", href: "/customers" });
        recentOrders.slice(0, 3).forEach((o) => {
            items.push({ type: "order", message: `${o.orderNumber} — ${formatINR(o.netAmount)} from ${o.customer?.name ?? "Unknown"}`, action: "View", color: "var(--gold)", href: `/orders/${o.id}` });
        });
        if (items.length === 0) items.push({ type: "info", message: "No alerts right now. Everything looks good!", action: "", color: "var(--green-bright)" });
        return items;
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-4">
                        <h2 suppressHydrationWarning className="text-lg font-semibold" style={{ fontFamily: "var(--font-playfair)" }}>
                            {greeting()}, {user?.firstName ?? "there"}. Here&apos;s your day.
                        </h2>
                    ...
                    <div className="grid sm:grid-cols-3 gap-4">
                        {[
                            { icon: "📦", text: `${todayOrders} orders today worth ${formatINR(todayRevenue)}`, action: "View Orders", href: "/orders" },
                            { icon: "💰", text: `${formatINR(outstanding)} outstanding collections`, action: "View Plan", href: "/customers" },
                            { icon: "📊", text: `${formatINR(monthRevenue)} revenue this month from ${monthOrders} orders`, action: "See Report", href: "/analytics" },
                        ].map((item, i) => (
                    ...
                    <div className="h-56">
                        {chartData.length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center text-sm text-[var(--text-muted)]">
                                {isLoading ? "Loading sales data..." : "No sales data for this period. Create orders to see trends."}
                            </div>
                        )}
                    </div>
                </div>
                                ...
                                <div className="flex items-start gap-3 p-3 rounded-lg bg-[var(--bg-secondary)]/50 border border-[var(--border)] hover:bg-[var(--bg-card-hover)] transition">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs text-[var(--text-muted)] flex items-center gap-1"><Clock size={10} />Now</span>
                ...
                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-md)] p-5">
                    ...
                    <div className="space-y-3">
                        {topProducts.length > 0 ? topProducts.map((p, i) => {
                            ...
                            <p className="text-sm text-[var(--text-muted)]">No product data yet</p>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-md)] p-5">
                    ...
                    <div className="space-y-3">
                        {topCustomers.length > 0 ? topCustomers.map((c, i) => (
                            <Link key={c.id} href={`/customers/${c.id}`}>
                                <div className="flex items-center gap-3 hover:bg-[var(--bg-card-hover)] rounded-lg p-1 -m-1 transition">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-[var(--text-muted)]">Outstanding: {formatINR(c.outstandingAmount ?? 0)}</p>
                                    </div>
                            ...
                            <p className="text-sm text-[var(--text-muted)]">No customer data yet</p>
                        )}
                    </div>
                </div>
            </div>
